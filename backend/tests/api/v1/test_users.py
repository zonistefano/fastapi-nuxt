...
def test_update_user_me_success_password(
    ...
    r_new = client.post(f"{settings.API_V1_STR}/login/oauth", data=login_data_new)
    assert r_new.status_code == 200

    # Verify login with old password fails
    login_data_old = {"username": original_email, "password": original_password}
    r_old = client.post(f"{settings.API_V1_STR}/login/oauth", data=login_data_old)
...
def test_read_user_by_id_success(
    ...
    response = client.get(
        f"{settings.API_V1_STR}/users/{user_to_get.id}",
        headers=superuser_token_headers,
    )
    assert response.status_code == 200
    user_data = response.json()
...
def test_update_user_by_id_success(
    ...
    update_data = {
        ...
        # Can also include password, is_active, is_superuser here if needed
    }
    ...
    # Verify DB changes
    db.expire_all()
    db_user = db.get(models.User, user_to_update.id)  # Fetch by primary key
    assert db_user is not None
...
def test_delete_user_by_id_success(
    ...
    # Verify user exists before deletion
    user_before = db.get(models.User, user_id)
    assert user_before is not None
    ...
    # Verify user is deleted from DB
    db.expire_all()
    user_after = db.get(models.User, user_id)
    assert user_after is None
...
def test_read_all_users_pagination(
    ...
    response_p0 = client.get(
        f"{settings.API_V1_STR}/users/?page=0",
        headers=superuser_token_headers,
    )
    ...
    response_p1 = client.get(
        f"{settings.API_V1_STR}/users/?page=1",
        headers=superuser_token_headers,
    )
...
def test_toggle_user_state_activate(
    ...
    response = client.post(
        f"{settings.API_V1_STR}/users/toggle-state",
        headers=superuser_token_headers,
        json=toggle_data,
    )
    assert response.status_code == 200
