...
from app.core.security import create_magic_tokens, verify_password
...
def test_create_user_profile_success(client: TestClient, db: Session, monkeypatch):
    ...
    response = client.post(f"{settings.API_V1_STR}/login/signup", json=data)
    ...
    user = crud.user.get_by_email(db, email=test_email)
...
@patch("app.api.v1.endpoints.login.send_magic_login_email")
def test_login_with_magic_link_existing_user(
    ...
    response = client.post(f"{settings.API_V1_STR}/login/magic/{user.email}")
    assert response.status_code == status.HTTP_200_OK
    content = response.json()
...
def test_login_with_magic_link_new_user_registration_open(
    ...
    response = client.post(f"{settings.API_V1_STR}/login/magic/{new_email}")
    assert response.status_code == status.HTTP_200_OK
    content = response.json()
    assert "claim" in content

    # Verify user was created
    user = crud.user.get_by_email(db, email=new_email)
    assert user is not None
...
def test_validate_magic_link_success(client: TestClient, db: Session):
    ...
    response = client.post(
        f"{settings.API_V1_STR}/login/claim",
...
def test_login_with_oauth2_success(client: TestClient, db: Session):
    ...
    response = client.post(
        f"{settings.API_V1_STR}/login/oauth", data=login_data
    )  # Form data
...
def test_login_with_oauth2_incorrect_password(client: TestClient, db: Session):
    ...
    response = client.post(f"{settings.API_V1_STR}/login/oauth", data=login_data)
...
def test_request_new_totp(
    ...
    response = client.post(
        f"{settings.API_V1_STR}/login/new-totp", headers=normal_user_token_headers
    )
    assert response.status_code == status.HTTP_200_OK
    content = response.json()
...
# - For PUT/DELETE, potentially mock `crud.user.authenticate`.
...
def test_refresh_token_success(client: TestClient, db: Session):
    ...
    with freeze_time(initial_time) as freezer:
        ...
        refresh_response = client.post(
            f"{settings.API_V1_STR}/login/refresh", headers=refresh_headers
        )
...
def test_revoke_token_success(client: TestClient, db: Session):
    ...
    assert user.email is not None
    login_data = {"username": user.email, "password": test_password}
    login_response = client.post(f"{settings.API_V1_STR}/login/oauth", data=login_data)
    assert login_response.status_code == status.HTTP_200_OK
    refresh_token = login_response.json()["refresh_token"]

    revoke_headers = {"Authorization": f"Bearer {refresh_token}"}
    revoke_response = client.post(
        f"{settings.API_V1_STR}/login/revoke", headers=revoke_headers
    )

    assert revoke_response.status_code == status.HTTP_200_OK
    content = revoke_response.json()
    assert content["msg"] == "Token revoked"

    refresh_again_response = client.post(
        f"{settings.API_V1_STR}/login/refresh", headers=revoke_headers
    )
...
@patch("app.api.v1.endpoints.login.send_reset_password_email")
def test_recover_password_existing_user(
    ...
    response = client.post(f"{settings.API_V1_STR}/login/recover/{user.email}")
...
def test_recover_password_non_existent_user(client: TestClient, monkeypatch):
    ...
    response = client.post(f"{settings.API_V1_STR}/login/recover/{non_existent_email}")
...
@patch("app.utilities.send_reset_password_email")
...
def test_reset_password_success(client: TestClient, db: Session):
    ...
    response = client.post(
        f"{settings.API_V1_STR}/login/reset",
        headers={"Authorization": f"Bearer {email_token}"},
        json=data,
    )
...
def test_access_token_expiration(client: TestClient, db: Session):
    ...
    with freeze_time(initial_time) as freezer:
        ...
        response = client.post(
            f"{settings.API_V1_STR}/login/new-totp", headers=expired_headers
        )
...
def test_refresh_token_expiration(client: TestClient, db: Session):
    ...
    with freeze_time(initial_time) as freezer:
        ...
        response = client.post(
            f"{settings.API_V1_STR}/login/refresh", headers=expired_headers
        )
