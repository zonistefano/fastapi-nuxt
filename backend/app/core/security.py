...
totp_factory = TOTP.using(
    secrets={"1": settings.TOTP_SECRET_KEY},
    issuer=settings.FRONTEND_HOST,
    alg=settings.TOTP_ALGO,
)
...
def create_new_totp(*, label: str, uri: str | None = None) -> NewTOTP:
    ...
    return NewTOTP(
        **{
            "secret": totp.to_json(),
            "key": totp.pretty_key(),
            "uri": totp.to_uri(issuer=settings.FRONTEND_HOST, label=label),
        }
    )


def verify_totp(
    *, token: str, secret: str = "", last_counter: int | None = None
) -> int | bool:
    """
    token: from user
...
def verify_password(*, plain_password: str | None, hashed_password: str | None) -> bool:
    if plain_password is None or len(plain_password) > 64:
        return False
